...
**Что нужно:**
- Добавить `.idea/`, `backups/`, `build/` (любые) в `.gitignore`
- Убрать дублирующиеся entries (`local.properties` два раза, частичные `/.idea/...`)
- `git rm --cached` для уже закоммиченных файлов из этих папок (если есть)

**Источник:** ex-KAN-..., замечание из B-1 PR (untracked .idea/ и backups/ мешали git add).
